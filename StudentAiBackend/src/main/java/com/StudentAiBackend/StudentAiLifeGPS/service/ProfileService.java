...
public class ProfileService {
    ...
    public Map<String, Object> getStudentProfileState(User user) {
        updateSkillProgression(user);
        Map<String, Object> state = new LinkedHashMap<>();

        // 1. Basic details
        Map<String, Object> basicDetails = new HashMap<>();
        basicDetails.put("username", user.getUsername());
        basicDetails.put("email", user.getEmail());
        basicDetails.put("role", user.getRole());
        basicDetails.put("level", user.getLevel());
        basicDetails.put("xp", user.getXp());
        basicDetails.put("coins", user.getCoins());
        basicDetails.put("streak", user.getStreak());
        basicDetails.put("activeMission", user.getActiveMission());
        basicDetails.put("diagnosticComplete", user.isDiagnosticComplete());
        ...
        state.put("targetCareer", user.getTargetCareer() != null ? user.getTargetCareer() : (goals.isEmpty() ? "Student Explorer" : goals.get(0).getGoalName()));
        ...
        List<UserSchedule> scheduleList = userScheduleRepository.findByUser(user);
        ...
        state.put("learningPreferences", user.getLearningPreferences() != null ? user.getLearningPreferences() : Collections.emptyList());
        state.put("targetCompletionDate", user.getTargetCompletionDate() != null ? user.getTargetCompletionDate() : LocalDate.now().plusWeeks(10).toString());
        state.put("projectsInterestedIn", user.getInterestedProjects() != null ? user.getInterestedProjects() : Collections.emptyList());
        ...
        state.put("strengths", user.getStrengths() != null ? user.getStrengths() : Collections.emptyList());
        state.put("weaknesses", user.getWeaknesses() != null ? user.getWeaknesses() : Collections.emptyList());
        state.put("recommendedDomains", user.getRecommendedDomains() != null ? user.getRecommendedDomains() : Collections.emptyList());
        ...
        List<String> badges = user.getDefeatedBosses() != null ? new ArrayList<>(user.getDefeatedBosses()) : new ArrayList<>();
        if (completedNodeIds.contains("python") && !badges.contains("Python Hero")) {
            badges.add("Python Hero");
        }
        if (user.getLevel() >= 3 && !badges.contains("DSA Master")) {
            badges.add("DSA Master");
        }
        if (user.getLevel() >= 5 && !badges.contains("Architect Elite")) {
            badges.add("Architect Elite");
        }
        state.put("unlockedBadges", badges);

        // 11. Career Readiness Report
        CareerReadiness currentReadiness = refreshCareerReadiness(user, false);
        Map<String, Object> crReport = buildCareerReadinessReport(user, currentReadiness);
        state.put("careerReadiness", crReport);

        // 12. Bosses Defeated Arena details
        ...
        if (skills.isEmpty()) {
            ...
            String[] bossNames = {"React Boss", "Java Boss", "DSA Boss", "Python Boss", "ML Boss"};
            ...
            for (int i = 0; i < skills.size(); i++) {
                ...
                String bid = s.getSkillName().toLowerCase();
                Map<String, Object> boss = new HashMap<>();
                boss.put("id", bid);
                boss.put("name", s.getSkillName() + " Boss");
                
                int rank = levelRank(s.getCurrentLevel());
                int reward = 200 + rank * 40;
                boss.put("xpReward", reward);

                String status = "Locked";
                if (listDefeated.contains(bid)) {
                    status = "Defeated";
                } else {
                    if (i == 0 || listDefeated.contains(skills.get(i - 1).getSkillName().toLowerCase())) {
                        status = "Ready";
                    }
                }
                boss.put("status", status);
        ...
        state.put("recoveryPlan", getOrGenerateRecoveryPlan(user, sessions, skills, crReport));
